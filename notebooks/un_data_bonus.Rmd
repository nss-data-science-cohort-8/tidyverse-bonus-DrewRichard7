...
ggplot(algebras_reshaped, aes(x = `Algebra I`, y = `Algebra II`)) +
  ...
  theme_minimal() + 
  annotate("text", x=60, y=40, label= sprintf('correlation = %s', correlation))
```
...
```{r}
school_testing |> 
  filter(school_name =='Adamsville Junior/Senior High School', subject=='Algebra I')
```

```{r}
algebras <- school_testing |> 
  filter((subject == 'Algebra I'|subject =='Algebra II') & grade == 'All Grades') |> 
  drop_na(pct_adv) |> 
  select(school_name, subject, pct_adv)
algebras <- algebras |> 
  group_by(school_name, subject) |> 
  summarise(pct_adv = mean(pct_adv, na.rm = TRUE), .groups = "drop")
